...
/**
 * Create and manage an modal for adding or editing notes. The modal allows users to input a note's title and text, and provides functionality to submit and save the note.
 * 
 * @param {string} [title='Untitled'] - The default title for the note. 
 * @param {string} [text='Add your note ...'] - The default text for the note.
 * @param {string} [title=''] - The time associated with the note.
 * @returns {Object} - An object containing functions to open the modal, class the modal, and handle note submission.
 */
const NoteModal = function (title = 'Untitled', text = 'Add your note ...', time = '') {
    ...
    $modal.innerHTML = `
     <button class="icon-btn large" aria-label="Close modal" data-close-btn>
        ...
        <input type="text" placeholder="Untitled" value="${title}" class="modal-title
        text-title-medium" data-note-field>
